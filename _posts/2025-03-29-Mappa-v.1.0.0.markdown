---
layout: post
title:  "Mappa the source generator C# mapper"
date:   2025-03-29 21:16:03 +0100
categories: projects csharp nuget source-generator mappa
---

[Mappa](https://github.com/sanelli/Mappa) (Italian for _map_) is a .NET source generator for C#. Mappa allows you to automatically generate mappers between different types, similarly to what AutoMapper does but the mapping generation happens at compile time.

## Why Mappa?
Mappa is a relatively new project (well, I have been working on this for a couple of years) and does not aim at replacing bigger players in this field, like AutoMapper. So why create a new mapping tool? Well because there was no automatic mapper that was satisfying my needs:
...
- I needed a mapper that takes into account protobuf
- I needed a mapper that was easier to configure, yet powerful
...
This will create mapping code like the following:
```csharp
namespace Sample;
...
- Improve performance by allowing users to map using `Span<T>` and friends
...
- Allow users to use `Add` method instead of using the `Indexer` when mapping dictionaries
- Add default rules to select the source property
- Support mapping to/from fields

## NuGet packages
Mappa provides 6 NuGet packages:
- [Mappa](https://www.nuget.org/packages/Mappa/): source generator that allows you to automatically generate mappings between classes and value types;
- [Mappa source generator](https://www.nuget.org/packages/Mappa.Generator/): source generator that allows you to automatically generate mappings between classes and value types;
