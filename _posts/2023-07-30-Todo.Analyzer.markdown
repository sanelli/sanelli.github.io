...
[Comment TODO Analyzer](https://github.com/sanelli/Todo.Analyzer) is a .NET analyzer for C#; it ensures that all TODO comments in your code respect the same standards.

## Options
These options can be set up in the `.editorconfig`:
...
- `todo_analyzer.comment.format.custom.regex`
    - Description: The regular expression used to check if the comment matches the expected criteria when `todo_analyzer.comment.format` is `custom`.
    - Default: `^ TODO .*$`
